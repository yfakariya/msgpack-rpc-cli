...
{
	/// <summary>
	///		Represents parser state when the start tag found.
	/// </summary>
	internal sealed class StartTagFoundState : SvcDirectiveParserState
	{
		...
		protected sealed override SvcDirectiveParserState ParseCore( char currentChar, TextReader nextReader )
		{
			if ( currentChar == '%' )
			{
				return new ServerDirectiveIndicatorFoundState( this );
			}
			else
			{
