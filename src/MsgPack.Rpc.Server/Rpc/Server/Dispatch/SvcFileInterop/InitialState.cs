...
{
	/// <summary>
	///		Represents initial (and skipping leading white spaces) parser state.
	/// </summary>
	internal sealed class InitialState : SvcDirectiveParserState
	{
		public InitialState() : base( null ) { }

		protected sealed override SvcDirectiveParserState ParseCore( char currentChar, TextReader nextReader )
		{
			if ( currentChar == '<' )
			{
				return new StartTagFoundState( this );
			}
			else
			{
