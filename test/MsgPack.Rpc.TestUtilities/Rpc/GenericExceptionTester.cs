...
#pragma warning restore 0612
			...
			try
			{
				var innerMessage = Guid.NewGuid().ToString();
				var message = Guid.NewGuid().ToString();
				workerDomain.SetData( "MsgPack.Rpc.GenericExceptionTester.InnerMessage", innerMessage );
				workerDomain.SetData( "MsgPack.Rpc.GenericExceptionTester.Message", message );
				workerDomain.SetData( "MsgPack.Rpc.GenericExceptionTester.Proxy", this );
				workerDomain.DoCallBack( TestSerializationOnPartialTrustCore );

				var target = workerDomain.GetData( "MsgPack.Rpc.GenericExceptionTester.Target" ) as T;
		...
		{
			var innerMessage = AppDomain.CurrentDomain.GetData( "MsgPack.Rpc.GenericExceptionTester.InnerMessage" ) as string;
			var message = AppDomain.CurrentDomain.GetData( "MsgPack.Rpc.GenericExceptionTester.Message" ) as string;
			var instance = AppDomain.CurrentDomain.GetData( "MsgPack.Rpc.GenericExceptionTester.Proxy" ) as GenericExceptionTester<T>;
			var target = instance.CreateTargetInstance( message, new Exception( innerMessage ) );
			Assert.That( target, Is.Not.Null );
			Assert.That( target.Message, Is.EqualTo( target.Message ) );
			Assert.That( target.InnerException, Is.Not.Null.And.TypeOf( typeof( Exception ) ) );
			Assert.That( target.InnerException.Message, Is.EqualTo( target.InnerException.Message ) );
			AppDomain.CurrentDomain.SetData( "MsgPack.Rpc.GenericExceptionTester.Target", target );
		}
